...
def search():
    ...
    if q == '':
        flash('Добавьте ключевое слово к фото, пользователя или тег, Enter keyword about photo, user or tag.', 'warning')
...
def read_all_notification():
    for notification in current_user.notifications:
        notification.is_read = True
    db.session.commit()
    flash('Все уведомления перемещены в архив. All notifications archived.', 'success')
    return redirect(url_for('.show_notifications'))
...
def photo_next(photo_id):
    ...
    if photo_n is None:
        flash('Это последнее фото. This is already the last one.', 'info')
...
def photo_previous(photo_id):
    ...
    if photo_p is None:
        flash('Это первое фото. This is already the first one.', 'info')
        return redirect(url_for('.show_photo', photo_id=photo_id))
...
def collect(photo_id):
    ...
    if current_user.is_collecting(photo):
        flash('Уже в коллекции. Already collected.', 'info')
        return redirect(url_for('.show_photo', photo_id=photo_id))

    current_user.collect(photo)
    flash('Фото в коллекции. Photo collected.', 'success')
...
def uncollect(photo_id):
    ...
    if not current_user.is_collecting(photo):
        flash('Еще не в коллекции. Not collect yet.', 'info')
        return redirect(url_for('.show_photo', photo_id=photo_id))

    current_user.uncollect(photo)
    flash('Фото убрано из коллекци. Photo uncollected.', 'info')
    return redirect(url_for('.show_photo', photo_id=photo_id))
...
def report_comment(comment_id):
    ...
    comment.flag += 1
    db.session.commit()
    flash('О комментарий сообщено. Comment reported.', 'success')
...
def report_photo(photo_id):
    ...
    photo.flag += 1
    db.session.commit()
    flash('О фото сообщено. Photo reported.', 'success')
...
def edit_description(photo_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Описание обновлено. Description updated.', 'success')
...
def new_comment(photo_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Комментарий опубликован. Comment published.', 'success')
    ...
    if form.validate_on_submit():
        for name in form.tag.data.split():
            ...
            if tag not in photo.tags:
                ...
                db.session.commit()
        flash('Тег добавлен. Tag added.', 'success')
...
def set_comment(photo_id):
    ...
    if photo.can_comment:
        ...
        flash('Комментирование отключено. Comment disabled', 'info')
    else:
        photo.can_comment = True
        flash('Комментирование включено. Comment enabled.', 'info')
    db.session.commit()
    return redirect(url_for('.show_photo', photo_id=photo_id))
...
def delete_photo(photo_id):
    ...
    db.session.commit()
    flash('Фото удалено. Photo deleted.', 'info')
...
def delete_comment(comment_id):
    ...
    db.session.commit()
    flash('Комментарий удален. Comment deleted.', 'info')
    return redirect(url_for('.show_photo', photo_id=comment.photo_id))
